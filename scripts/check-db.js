...
(async () => {
  try {
    ...
    if (colRes.rowCount > 0) {
      ...
      const sample = await client.query(`SELECT id, name, created_at FROM dresses ORDER BY created_at DESC LIMIT 5`);
      console.log('SAMPLE_ROWS_NO_POSITION:', JSON.stringify(sample.rows, null, 2));
    }
    await client.end();
  } catch (err) {
